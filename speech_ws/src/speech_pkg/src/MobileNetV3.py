...
from tensorflow.keras.layers import GlobalAveragePooling2D, Reshape, Dense, multiply, Softmax, Flatten, Permute, AvgPool2D
...
class MobileNetV3():
    ...
    def __global_depthwise_block(self, _inputs):
        assert _inputs.get_shape().as_list()[1] == _inputs.get_shape().as_list()[2]
        kernel_size = _inputs.get_shape().as_list()[1]
    ...
    def __se_block(self, _inputs, ratio=4, pooling_type='avg'):
        channel_axis = 1 if K.image_data_format() == "channels_first" else -1
        filters = _inputs.get_shape().as_list()[channel_axis]
    ...
    def build(self,input_shape=(224, 224, 3), num_classes=1000, model_type='large', pooling_type='avg', include_top=True, weights=None, input_tensor=None):
        ...
        pooled_shape = (1, 1, net.get_shape().as_list()[-1])
        net = Reshape(pooled_shape)(net)
        net = Conv2D(1024 if model_type is 'large' else 1024, (1, 1), strides=(1, 1), padding='same', use_bias=True,name='conv_2')(net)
        net = Activation(self.Hswish,name='conv_2_activation')(net)
        
        if include_top:
            y = Conv2D(num_classes,(1,1),strides=(1,1),padding='same',use_bias=True,name='logits')(net)
            y = Flatten()(y)
            y = Softmax()(y)
